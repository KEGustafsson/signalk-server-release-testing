...
class AdminUiTester {
  ...
  async screenshot(name) {
    ...
    console.log(`Taking screenshot: ${name} at ${isoString}`);
    //const filename = `${name}-${Date.now()}.png`;
    const filename = `${name}-${isoString.replace(/[:.]/g, '-')}.png`;
  ...
  /**
   * Navigate to admin page by clicking sidebar menu items
   * This works properly with React Router SPAs
   */
  async navigateViaMenu(menuPath) {
    ...
    // Navigate by clicking menu items
    const parts = menuPath.split(' > ');
    for (let i = 0; i < parts.length; i++) {
      const menuItem = parts[i];
      try {
        // Find and click the menu item
        const link = await this.page.locator(`text="${menuItem}"`).first();
        await link.click({ timeout: 5000 });
        await this.page.waitForTimeout(i === parts.length - 1 ? 2000 : 500);
      } catch (e) {
        console.log(`Could not click menu item: ${menuItem}`);
      }
    }
  ...
  /**
   * Test Dashboard (Admin Dashboard with stats)
   */
  async testDashboard() {
    ...
    try {
      ...
      // Navigate to Dashboard via sidebar menu
      await this.navigateViaMenu('Dashboard');

      // Close any popups/modals that might be open (like "What's new")
      try {
        // Try clicking close button with X
        const closeButtons = await this.page.$$('button');
        for (const btn of closeButtons) {
          const text = await btn.textContent();
          if (text && (text.includes('×') || text.includes('✕') || text.trim() === 'X')) {
            await btn.click();
            await this.page.waitForTimeout(300);
            break;
          }
      ...
      // Check for dashboard stats content
      const hasDashboardContent = await this.page.evaluate(() => {
      ...
      if (hasDashboardContent) {
        results.passed.push('Dashboard shows server statistics');
      } else {
        results.warnings.push('Dashboard statistics may not be visible');
      }

      // Check for connection & plugin status table
      const hasStatusTable = await this.page.$('table, [class*="status"], [class*="connection"]');
      if (hasStatusTable) {
        results.passed.push('Connection & Plugin status visible');
      }
  ...
  async testDataBrowser() {
    ...
    try {
      ...
      // Navigate to Data Browser via sidebar menu
      await this.navigateViaMenu('Dashboard > Data Browser');
      await this.page.waitForTimeout(1000); // Extra time for data to load
      ...
      try {
        await this.page.waitForSelector(
          '[class*="tree"], [class*="data"], table, [class*="json"], pre',
          { timeout: 10000 }
        );
  ...
  async testPluginManagement() {
    ...
    try {
      ...
      // Navigate to Plugin Config via sidebar menu (Server > Plugin Config)
      await this.navigateViaMenu('Dashboard > Server > Plugin Config');
      await this.page.waitForTimeout(1000); // Extra time for plugins to load
      ...
      await this.screenshot('plugin-config');
    } catch (e) {
      results.failed.push(`Plugin config test error: ${e.message}`);
      await this.screenshot('plugin-config-error');
    }
  ...
  async testSecurityUsers() {
    ...
    try {
      ...
      // Navigate to Security Users via sidebar menu (Security > Users)
      await this.navigateViaMenu('Dashboard > Security > Users');
      ...
      if (hasSecurityOptions) {
        results.passed.push('User options visible');
      } else {
        results.warnings.push('User options may not be displayed');
      }

      await this.screenshot('security-users');
    } catch (e) {
      results.failed.push(`Security users test error: ${e.message}`);
      await this.screenshot('security-users-error');
    }
  ...
  async testSecuritySettings() {
    ...
    try {
      ...
      // Navigate to Security Users via sidebar menu (Security > Users)
      await this.navigateViaMenu('Dashboard > Security > Settings');
  ...
  async testServerConfiguration() {
    ...
    try {
      ...
      // Navigate to Server Settings via sidebar menu (Server > Settings)
      await this.navigateViaMenu('Dashboard > Server > Settings');
      ...
      await this.screenshot('server-settings');
    } catch (e) {
      results.failed.push(`Server settings test error: ${e.message}`);
      await this.screenshot('server-settings-error');
    }
  ...
  async testConnectionManagement() {
    ...
    try {
      ...
      // Navigate to Data Connections via sidebar menu (Server > Data Connections)
      await this.navigateViaMenu('Dashboard > Server > Settings > Data Connections');
      await this.page.waitForTimeout(1000); // Extra time for connections to load
      ...
      await this.screenshot('data-connection');
    } catch (e) {
      results.failed.push(`Data connection test error: ${e.message}`);
      await this.screenshot('data-connection-error');
    }
  ...
  async testWebAppStore() {
    ...
    try {
      ...
      // Navigate to Appstore via sidebar menu
      await this.navigateViaMenu('Dashboard > Appstore');
