...
class ContainerManager {
  ...
  async prepare() {
    ...
    // Set permissions to allow container's node user (UID 1000) to write
    // The SignalK Docker image runs as node user which may have different UID than host
    ...
    if (this.configPath && await fs.pathExists(this.configPath)) {
      await fs.copy(this.configPath, path.join(this.dataDir, 'settings.json'));
    } else {
      ...
      await fs.writeJson(path.join(this.dataDir, 'settings.json'), defaultSettings, { spaces: 2 });
    }
  }
  ...
  async start(config = {}) {
    ...
    const containerConfig = {
      ...
      HostConfig: {
        ...
        Binds: [`${this.dataDir}:/home/node/.signalk`],
  ...
  async waitForReady(timeout = null) {
    ...
    throw new Error(
      `Server failed to start within ${timeout}ms. Last logs:\n${logs}`
    );
  }
