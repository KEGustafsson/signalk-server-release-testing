...
class ContainerManager {
  ...
  async prepare() {
    ...
    // Create security.json with pre-configured admin user
    // This ensures security is enabled and we can log in without going through the "Enable Security" UI
    // The password hash is for "admin123" using bcrypt with 10 rounds
    const securityPath = path.join(this.dataDir, 'security.json');
      ...
      users: [
        {
          ...
          username: 'admin',
          type: 'admin',
          // bcrypt hash of "admin123" with $2a$ prefix for maximum compatibility
          password: '$2a$10$1nWvGdZgsPXDmFP.QCvMTOGB1q16UXuhSG3sqLqWMu6F619eQVyCS',
        },
      ],
    ...
    await fs.writeJson(securityPath, securityConfig, { spaces: 2 });
    ...
    await fs.chmod(securityPath, 0o666);
  }
  ...
  /**
   * Copy settings.json and security.json into the container
   */
  async copySettingsToContainer() {
    const settingsPath = path.join(this.dataDir, 'settings.json');
    const securityPath = path.join(this.dataDir, 'security.json');
    const settings = await fs.readJson(settingsPath);
    const security = await fs.readJson(securityPath);

    // Create a tar archive with both settings.json and security.json files
    const tar = require('tar-stream');
    const pack = tar.pack();

    pack.entry({ name: 'settings.json' }, JSON.stringify(settings, null, 2));
    pack.entry({ name: 'security.json' }, JSON.stringify(security, null, 2));
    pack.finalize();
